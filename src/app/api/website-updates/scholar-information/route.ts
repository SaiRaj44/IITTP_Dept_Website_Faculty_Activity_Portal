// app/api/website/scholars/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const item = await ScholarInformation.findOne({
        _id: id,
    ...
    const items = await ScholarInformation.find(filterQuery)
      .sort({ year: -1, batch: -1, name: 1 })
      .skip(skip)
      .limit(limit);
...
const handler = createActivityHandler({
  ...
  searchFields: ["name", "domain", "category", "year", "batch"],
  defaultSort: { year: -1, batch: -1, name: 1 },
});
